...
export default function ViewGroup() {
      ...
      .then((data) => {
        if (data.status === 400) {
          ...
          if (data.status !== "error") {
            ...
            addBook({ title: "", description: "", fileDownload: "", pages: 0 }); // still fix this
            Swal.fire({
  ...
  if (isAddBook === false) {
    return (
      <div>
        <div className="d-flex m-10 justify-content-between">
          <button
        ...
        <div className="container">
          ...
          >
            <div className="text-success">Total books: {books? books.filter(book=>book.groupId === group.id).map(book=>book).length: 'waiting'}</div>
    ...
    return (
      <div>
        ...
        <div className="bg-dark text-white text-center mb-5 ">
          {" "}
          <h1>Add Book</h1>
        </div>
        <div className="container">
